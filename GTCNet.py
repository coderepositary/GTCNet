...
option="/storage/liujinhuan/horse2zebra/horse2zebra/new_ijk_data1/"
...
text_x = pickle.load(open('/storage/liujinhuan/liujinhuan/cloth.binary.p', 'rb'),encoding='iso-8859-1')
    ...
    for lamda in [1]:
        for n_hidden in [2**8]:
            ...
            with tf.variable_scope("Model") as scope:
                enc_A,enc_Bg,fake_B = build_generator_resnet_6blocks(batch_size,input_A, "generator_encoder_A2Bg"+str(count))
                rec_B = build_gen_discriminator(input_B, "discriminator_B"+str(count)) #判别A
                fake_rec_B = build_gen_discriminator(fake_B, "discriminator_Bg"+str(count)) #判别生成的B

                _,__,cyc_A= build_generator_resnet_6blocks(batch_size,fake_B, "generator_Ag"+str(count)) #从生成的B到生成A
                rec_A = build_gen_discriminator(input_A, "discriminator_A"+str(count)) #判别A
                fake_rec_A = build_gen_discriminator(cyc_A, "discriminator_Ag"+str(count)) #判别生成的B

                enc_B = build_encoder(batch_size,input_B,"generator_encoder_B"+str(count))
                scope.reuse_variables()
                enc_B1 = build_encoder(batch_size,input_B1,"generator_encoder_B"+str(count))

            intra_sim_ij = tf.reduce_sum(tf.abs(enc_Bg-enc_B),[1, 2,3])
            intra_sim_ik = tf.reduce_sum(tf.abs(enc_Bg-enc_B1),[1, 2,3])  #1*batch
            enc_A_W = tf.get_variable('generator_A_W'+str(count), [128,n_hidden],dtype=tf.float32,trainable=True)
            enc_A_b = tf.get_variable('generator_A_b'+str(count), [n_hidden],dtype=tf.float32,trainable=True)
            enc_B_W = tf.get_variable('generator_B_W'+str(count),[128,n_hidden],dtype=tf.float32,trainable=True)
            enc_B_b = tf.get_variable('generator_B_b'+str(count),[n_hidden],dtype=tf.float32,trainable=True)

            enc_A =tf.sigmoid(tf.matmul(tf.reshape(tf.nn.avg_pool(value = enc_A, ksize = [1,16, 16, 1], strides = [1,1, 1,1], padding = 'VALID'), [int(batch_size/3),-1]),enc_A_W)+enc_A_b)
            enc_B =tf.sigmoid(tf.matmul(tf.reshape(tf.nn.avg_pool(value = enc_B, ksize = [1,16, 16, 1], strides = [1, 1, 1,1], padding = 'VALID'), [int(batch_size/3),-1]),enc_B_W)+enc_B_b)
            enc_B1 =tf.sigmoid(tf.matmul(tf.reshape(tf.nn.avg_pool(value = enc_B1, ksize = [1,16, 16, 1], strides = [1,1, 1, 1], padding = 'VALID'), [int(batch_size/3),-1]),enc_B_W)+enc_B_b)

            cross_sim_ij_v = tf.reduce_sum(enc_A*enc_B,1)
            cross_sim_ij_c = tf.reduce_sum(text_output1*text_output2,1)
            cross_sim_ik_v = tf.reduce_sum(enc_A*enc_B1,1)
            cross_sim_ik_c = tf.reduce_sum(text_output1*text_output3,1)
            mij =0.01*intra_sim_ij + cross_sim_ij_v + lamda*cross_sim_ij_c
            mik =0.01*intra_sim_ik + cross_sim_ik_v + lamda*cross_sim_ik_c
            score=tf.subtract(mij,mik)

            # Op for calculating the loss
            with tf.name_scope("cross_ent"):
                cyc_loss = tf.reduce_mean(tf.abs(input_A-cyc_A))
                disc_loss = 0.5 *tf.reduce_mean(tf.squared_difference(fake_rec_B,1)) + 0.5 *tf.reduce_mean(tf.squared_difference(fake_rec_A,1))
                con_loss = tf.reduce_mean(tf.abs(fake_B-input_B))
                g_loss = disc_loss + 10*cyc_loss
                d_loss = 0.5 *tf.reduce_mean(tf.square(fake_rec_B)) + 0.5 *tf.reduce_mean(tf.squared_difference(rec_B,1))+\
                         0.5 *tf.reduce_mean(tf.square(fake_rec_A)) + 0.5 *tf.reduce_mean(tf.squared_difference(rec_A,1))
            ...
            with tf.name_scope("train"):
                ...
                # d_optim = tf.train.RMSPropOptimizer(learning_rate) #判别器训练器
                g_optim = tf.train.RMSPropOptimizer(learning_rate) #生成器训练器
                ...
                d_grads_and_vars = d_optim.compute_gradients(0.1*d_loss, var_list=d_A_vars) #计算判别器参数梯度
                d_train = d_optim.apply_gradients(d_grads_and_vars) #更新判别器参数
                g_grads_and_vars = g_optim.compute_gradients(1*g_loss+0.1*reg_loss+10000*con_loss+bpr_loss, var_list=g_A_vars) #计算生成器参数梯度
            ...
            with tf.Session(config=config) as sess:
                ...
                # saver.restore(sess, "tmp2/tune_alexnet/checkpoints/model_epoch156.ckpt")
                ...
                for epoch in range(num_epochs):
                    ...
                    for step in range(train_batches_per_epoch):
                        ...
                        [ms,train_op_,each_disc_loss,each_cyc_loss,each_g_loss,each_con_loss,each_d_loss,each_bpr_loss]= sess.run([merged_summary,train_op,disc_loss,cyc_loss,g_loss,con_loss,d_loss,bpr_loss],
                        ...
                        disc_loss_sum += each_disc_loss
                        cyc_loss_sum += each_cyc_loss
                        g_loss_sum += each_g_loss
                        con_loss_sum += each_con_loss
                        d_loss_sum += each_d_loss
                        bpr_loss_sum += each_bpr_loss

                    print("disc_loss_sum:{} cyc_loss_sum:{} g_loss_sum:{} con_loss_sum:{} d_loss_sum:{} bpr_loss_sum:{}".format(disc_loss_sum,cyc_loss_sum,g_loss_sum,con_loss_sum,d_loss_sum,bpr_loss_sum))
                    file.write("disc_loss_sum:{} cyc_loss_sum:{} g_loss_sum:{} con_loss_sum:{} d_loss_sum:{} bpr_loss_sum:{}\n".format(disc_loss_sum,cyc_loss_sum,g_loss_sum,con_loss_sum,d_loss_sum,bpr_loss_sum))
                    ...
                    if val_acc > best_validation_auc_score:
                        ...
                        for test_epoch in range(test_batches_per_epoch):
                            ...
                            text_batch_j = sess.run(next_batchj)
                            text_batch_k = sess.run(next_batchk)
                            input_A_value,input_B_value,fake_B_value, cyc_A_value,acc = sess.run([input_A,input_B,fake_B,cyc_A,accuracy], feed_dict={x: img_batch,
                                                                                                    text_x_i:text_batch_i,
                                                                                                    text_x_j:text_batch_j,
                                                                                                    text_x_k:text_batch_k,
                                                                                                    keep_prob:1.})
                            for k in range(int(batch_size/3)):
                                imsave("./output/fakeB/fakeB_"+ str(count_test)+".jpg",((fake_B_value[k]+1)*127.5).astype(np.float32))
                                imsave("./output/fakeB/cycA_"+ str(count_test)+".jpg",((cyc_A_value[k]+1)*127.5).astype(np.float32))
                                count_test += 1
                            test_acc += acc
                            test_count += 1
                        test_acc /= test_count
